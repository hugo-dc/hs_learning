...
myAction tempname temph = 
    do -- Start by displaying a greeting on the terminal
        ...
        -- hGetContents perform a lazy read of the entire file
...
{- This function takes two parameters: a filename pattern and another 
 - function. It will create a temporary file, and pass the name and Handle of
 - that file to the given function. 
 -
...
withTempFile pattern func = 
   do -- The library ref says that getTemporaryDirectory may raise on 
      ...
      -- So, we run getTemporaryDirectory under catch. catch takes two
      -- functions: one to run, and a different one to run if the first raised
      ...
      -- file. finally takes two actions. The first is the action to run. The
      -- second is an action to run after the first, regardless of whether the
      -- first action raised an exception. This way, we ensure the temporary
      -- file is always deleted. The return value from finally is the first
